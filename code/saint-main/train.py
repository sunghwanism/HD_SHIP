...
from data_openml import data_prep,task_dset_ids,DataSetCatCon
import argparse
from torch.utils.data import DataLoader
import torch.optim as optim
from utils import count_parameters, classification_scores, mean_sq_error, preprocessing, seed_everything
...
def train():
    ...
    cat_dims, cat_idxs, con_idxs, X_train, y_train, X_valid, y_valid, train_mean, train_std = data_prep(opt.dset_seed, train_x, train_y, test, category_lst, datasplit=[.80, .20])
    ...
    for epoch in range(opt.epochs):
        ...
        for i, data in enumerate(trainloader, 0):
            optimizer.zero_grad()
            # x_categ is the the categorical data, x_cpyont has continuous data, y_gts has ground truth ys. cat_mask is an array of ones same shape as x_categ and an additional column(corresponding to CLS token) set to 0s. con_mask is an array of ones same shape as x_cont. 
